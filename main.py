...
class Trainer:
    ...
    def train(self, start_step=1):
        ...
        for epoch in range(1, self.epochs + 1):
            for step, batch_data in enumerate(self.train_loader):
                ...
                print(f"【train】{epoch}/{self.epochs} step:{global_step}/{self.total_step} loss:{loss.item():.4f}")

                # 每 save_step 步保存模型和step
                if global_step % self.save_step == 0:
                    model_path = os.path.join(self.output_dir, "pytorch_model_ner.bin")
                    step_path = os.path.join(self.output_dir, "global_step.txt")
                    torch.save(self.model.state_dict(), model_path)
                    with open(step_path, "w") as f:
                        f.write(str(global_step))
                    print(f"已保存模型到 {model_path}, 当前step: {global_step}")

                global_step += 1

        # 最终保存
        torch.save(self.model.state_dict(), os.path.join(self.output_dir, "pytorch_model_ner.bin"))
        with open(os.path.join(self.output_dir, "global_step.txt"), "w") as f:
            f.write(str(global_step))
    ...
    def test(self):
        ...
        torch.cuda.empty_cache()  #  测试完释放显存
...
def main(data_name):
    ...
    with open(os.path.join(args.data_path, "train.txt"), "r", encoding="utf-8") as fp:
        train_data = [json.loads(d) for d in fp.read().split("\n") if d.strip()]
    with open(os.path.join(args.data_path, "dev.txt"), "r", encoding="utf-8") as fp:
        dev_data = [json.loads(d) for d in fp.read().split("\n") if d.strip()]
    ...
    del trainer.train_loader, trainer.dev_loader, trainer.optimizer, trainer.schedule
